...
class PDF(FPDF):
    def header_curriculo(self, dados):
        ...
        self.cell(0, 6, f"Site: {dados['contato']['site']}", new_x="LMARGIN", new_y="NEXT", align="C", link=dados["contato"]["site"])

        self.set_text_color(0, 0, 0)
        self.set_font("DejaVu", "B", 11)
        self.cell(0, 6, f"{dados['cargo']} | {dados['foco']}", new_x="LMARGIN", new_y="NEXT", align="C")
...
def gerar_pdf(dados):
    ...
    for ed in dados["educacao"]:
        ...
        pdf.cell(0, 6, f"{ed['curso']} - {ed['instituicao']} ({ed['periodo']})", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
